...
function Connection(ip, port) {
  ...
  this._socket.on("data", this._process.bind(this));
...
Connection.prototype._process = function(packet) {
  ...
  if (packet.length === expectedPacketLength) {
    var command = TcpCommands.typeForCode[packet.readUInt8(4)];
    var correlationId = uuid.unparse(packet, 5);
    console.log("Received " + command + " command with correlation id: " + correlationId);

    var payload = null;
    if (packet.length > 21) {
      payload = packet.slice(21);
    }
    ...
    this._process(packet.slice(0, expectedPacketLength));
    this._process(packet.slice(expectedPacketLength));
  } else {
